...
int main(void)
{
	...
	nRet = sendtoall(sockfdClient, strTemp, sizeof(char) * 32, 0, (struct sockaddr *)&stSockAddrInClient, clientLen);
	printf("Client (%s: %u) sendto %d bytes to server: %s (%d) \n", \
	...
	printf("Client (%s: %u) recvfrom %d bytes from server: %s (%d) \n", \
		inet_ntoa(stSockAddrInClient.sin_addr), ntohs(stSockAddrInClient.sin_port), nRet, strTemp, strlen(strTemp));
	close(sockfdClient);
	printf("Client done!! \n");
