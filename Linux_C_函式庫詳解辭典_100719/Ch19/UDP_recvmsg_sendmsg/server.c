...
int main(void)
{
	...
	while (1) {
		...
		printf("\nServer recvfrom %d bytes from client (%s: %u): %s (%d) \n", \
			nRet, inet_ntoa(stSockAddrInClient.sin_addr), ntohs(stSockAddrInClient.sin_port), strTemp, strlen(strTemp));
		strcat(strTemp, "World!! ");

		nRet = sendto(sockfdServer, (void *)strTemp, sizeof(char) * 32, 0, (struct sockaddr *)&stSockAddrInClient, clientLen);
		if (nRet == -1) {
			herror("sendto");
			continue;
		}

		printf("Server sendto %d bytes to client (%s: %u): %s (%d) \n", \
			nRet, inet_ntoa(stSockAddrInClient.sin_addr), ntohs(stSockAddrInClient.sin_port), strTemp, strlen(strTemp));
		break;
	}
	close(sockfdServer);
