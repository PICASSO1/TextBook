...
// schar wait queues. kernel newer than 2.3.0 uses a different syntax for declaring wait queues so check for that. 
...
static int schar_read_proc(ctl_table *ctl, int write, struct file *file, void *buffer, size_t *lenp);
...
static int schar_mmap(struct file *file, struct vm_area_struct *vma)
{
	...
	// remap user buffer
	if (remap_page_range(vma->vm_start, virt_to_phys(schar_buffer), size, vma->vm_page_prot))
		return -EAGAIN;
...
#if 0
// increment and decrement usage count when someone is accessing /proc/sys/dev/schar
...
static ssize_t schar_write(struct file *file, const char *buf, size_t count, loff_t *offset)
{
	...
	// if we were really writing - modify the file position to reflect the amount of data written
	file->f_pos += count;
