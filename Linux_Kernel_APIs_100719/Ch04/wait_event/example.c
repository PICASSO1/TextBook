 ...
 *
 * 3. 執行結果：
 *     a) 若只有條件成立 (condition == 1;)，例如：(IS_CONDITION == 2)的狀況。wait_event()依然不會被喚醒，要到卸載模組的時候才會被喚醒。
 *         這種行為當然是不合理的。
 *     b) 若使用wake_up_interruptible()，例如：(IS_CONDITION == 4)的狀況。wait_event()也不會被喚醒，要到卸載模組的時候才會被喚醒。
 *         這種行為也是不合理的。
 *     c) 若使用wake_up()，例如：(IS_CONDITION == 3)的狀況；但條件不成立 (condition == 0;)，把condition = 1; 這一行註解掉，因為wait_event()
 *         被喚醒了，但是條件並不成立，整個程序就會卡在那兒，只能直接重新開機。
 *     d) 結論：使用wait_event()函式，必須1. 條件成立 (condition == 1; )，且2. 使用wake_up(); 函式。兩者均成立才能夠正常地被喚醒！
 * 
 * x. (To do something....)
 *
(*)?*/
...
MODULE_DESCRIPTION("wait_event() function sample. ");
...
{
	condition = 0;
	printk(KERN_INFO "%s (%d): before wait_event() (jiffies: %ld; condition = %d) .... \n", __FUNCTION__, current->pid, jiffies, condition);
	wait_event(example_wq, condition);
	printk(KERN_INFO "%s (%d): after wait_event() (jiffies: %ld; condition = %d) .... \n", __FUNCTION__, current->pid, jiffies, condition);
