 ...
 * 
 * 3. 執行結果：
 *     a) 可用wake_up(); 和wake_up_interruptible(); 函式喚醒，回傳值為剩餘的秒數。
 *     b) 若只有條件成立 (condition == 1; )也不會被喚醒。
 *     c) 時間到了 (TimeOut)，或是接收到信號 (Signal)，既使條件不成立 (condition == 0; )也會被喚醒；TimeOut的回傳值為0，接收到Signal的回傳值
 *         卻是"-512"?? 這一點蠻奇怪的！
 * 
 * x. (To do something....)
 *
...
MODULE_DESCRIPTION("wait_event_interruptible_timeout() function sample. ");
...
static int wait_event_kthrFunC(void *num)
{
	...
	condition = 0;
	printk(KERN_INFO "%s (%d): before wait_event_interruptible_timeout() (jiffies: %ld; condition = %d; ret = %ld) .... \n", __FUNCTION__, current->pid, jiffies, condition, ret);
	ret = wait_event_interruptible_timeout(example_wq, condition, 15000);
	printk(KERN_INFO "%s (%d): after wait_event_interruptible_timeout() (jiffies: %ld; condition = %d; ret = %ld) .... \n", __FUNCTION__, current->pid, jiffies, condition, ret);
