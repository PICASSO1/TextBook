 ...
 * 
 * 2. 原本的sleep_on()改成用interruptible_sleep_on_timeout()，它的用法是：程序進入睡眠狀態之後，
 *     a) 時間到了就會被喚醒；b) 接收到信號Signal會被喚醒；c) 可以被wake_up()喚醒；d) wake_up_interruptible()也能喚醒。
 * 
 * 3. interruptible_sleep_on_timeout()函式的第2個參數，是要自動醒來的毫秒數 (0.001秒)；若是在中途被喚醒，則是回傳剩餘的毫秒數；若是因為Time
 *     Out而自動醒來則是回傳0。
 * 
 * 4. 除此之外，還有一系列的相關函式如下：
...
MODULE_DESCRIPTION("interruptible_sleep_on_timeout() function sample. ");
...
static int sleep_on_kthrFunC(void *num)
{
	...
	printk(KERN_INFO "%s (%d): before interruptible_sleep_on_timeout() (jiffies: %ld; ret = %ld) .... \n", __FUNCTION__, current->pid, jiffies, ret);
	ret = interruptible_sleep_on_timeout(&example_wq, 20000);	// 因為要花時間查詢PID，所以時間設長一些.... 
	printk(KERN_INFO "%s (%d): after interruptible_sleep_on_timeout() (jiffies: %ld; ret = %ld) .... \n", __FUNCTION__, current->pid, jiffies, ret);
