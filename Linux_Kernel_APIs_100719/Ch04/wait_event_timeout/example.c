 ...
 *
 * 3. 執行結果：
 *     a) 無法使用wake_up_interruptible(); 函式喚醒。
 *     b) 可以使用wake_up(); 函式喚醒；回傳值為剩餘的秒數。
 *     c) 只有條件成立 (condition == 1; )也不會喚醒。
 *     d) 若是因TimeOut時間到了，既使條件不成立 (condition == 0; )也會被喚醒。
 *     e) 結論：wait_event_timeout(); 函式，TimeOut的優先權大於Condition，既使條件不成立，只要時間到了就會被喚醒！
 * 
 * x. (To do something....)
 *
(*)?*/
...
MODULE_DESCRIPTION("wait_event_timeout() function sample. ");
...
static int wait_event_kthrFunC(void *num)
{
	...
	condition = 0;
	printk(KERN_INFO "%s (%d): before wait_event_timeout() (jiffies: %ld; condition = %d; ret = %ld) .... \n", __FUNCTION__, current->pid, jiffies, condition, ret);
	ret = wait_event_timeout(example_wq, condition, 5000); 
	printk(KERN_INFO "%s (%d): after wait_event_timeout() (jiffies: %ld; condition = %d; ret = %ld) .... \n", __FUNCTION__, current->pid, jiffies, condition, ret);
